...
POSITIVE_WHOAMI_REPLIES = ['норм чел', 'котик', 'милаха', 'няша',
                           'классный', 'приятный', 'хороший', 'крутой', 'классный', 'офигенный']

BYE = ['споки', 'спокойной ночи', 'а ну спать',
       'до завтра', 'пока', 'сладких снов']
...
GOOD_DAY = ['Привет!', 'Добрый день!', 'Ку :3', 'Как настроение?',
            'Охае', 'Шалом))0)', 'Э, салам алейкум, брат', 'Приветик)']
GOOD_EVENING = ['Привет!', 'Добрый вечер!', 'Ку :3', 'Как настроение?',
                'Охае', 'Шалом))0)', 'Э, салам алейкум, брат', 'Приветик)']
...
HAVE_A_GOOD_DAY = ['Покич', 'Хорошего дня)', 'Удачи тебе)',
                   'Буду ждать твоего сообщения)', 'Я буду скучать(', 'Удачи тебе сегодня']

WHATSUP_QUESTIONS = ['как дела?', 'как настроение?',
                     'как жизнь?', 'как твои дела?', 'что нового?', 'как ты?']
...
NEGATIVE_WAHATSUP_ANSWERS = ['Отвратительно',
                             'Ужасно(', 'Плоха, ты бака(', 'Не твое дело!']

APPEALS = ['кагуя', 'слушай', 'эй бейба', 'девка']
POSITIVE_APPEALS_ANSWERS = ['Да', 'А',
                            'Слушаю тебя, сенпай', 'Слушаю)', 'Что такое?']
NEGATIVE_APPEALS_ANSWERS = ['Чего тебе', 'Да.', 'С мамой своей поговори, а меня не трогай',
                            'Чего тебе, ебло?', 'Иди нахуй сразу', 'В мут с нулевой']

WHY = ['почему', 'зачем', 'схуяли']
POSITIVE_WHY_ANSWERS = ['Потому что)', 'Звёзды так сошлись',
                        'Потому что у меня такая классная жопа', 'Сенат США так решил', 'Ну прост', 'А почему бы и нет)']
NEGATIVE_WHY_ANSWERS = ['Потому что иди нахуй',
                        'Тя ебать не должно', 'Это инфа не для тебя', 'Потому что.']

MONTHS = ['января', 'февраля', 'марта', 'апреля', 'мая', 'июня',
          'июля', 'августа', 'сентября', 'октября', 'ноября', 'декабря']

OR_ANSWERS = ['Ну разумеется', 'Конечно же', 'Я думаю',
              'Мне кажется', 'Я выбираю', 'Мне больше нравится']
...
# вероятность ответа сообщением пользователя (из top_messages)
REPLY_WITH_USR_MSG = 0.35
...
# задержка в отправке сообщений, шобы на человека было похоже (в секундах)
SLEEP_TIME = 0.6
...
# контрольное сообщение, на которое можно ответить, если юзер нихуя не написал
CONTROL_MSGS = dict()
...
def update_timetable(msg, force=False):
    ...
    for sheet_num in range(len(workbook.sheet_names())):
        ...
        for col_num in range(num_cols):
            if (break_flag):
                break
            cell_value = worksheet.cell_value(1, col_num)
            if USERS[usr_id]['group'] == cell_value:
...
# dt ОБЯЗАТЕЛЬНО типа datetime.date, иначе пизды получишь
def get_nearest_pair_time(msg, dt):
...
# это я вообще на каком-то форуме подрезал
...
def get_stat(usr_id):  # выдаёт пользователю стату
    ...
    res += 'Картинок открыто: ' + \
        str(USERS[usr_id]['pics_unlocked']) + \
        '/' + str(len(glob('LEGS/*'))) + '\n'
    res += 'Настроение твоей Кагуи: ' + \
        str(round(USERS[usr_id]['mood'], 2))[:5] + '\n'
    return res
...
def set_wakeup_time_cmd(bot, update):
    ...
    try:
        ...
        try:
            base_get_up_time = datetime.datetime.strptime(
                inp_str, "%H %M").time()
        except:
            try:
                base_get_up_time = datetime.datetime.strptime(
...
def reply(bot, update):  # ответ на обычное сообщение
    ...
    # если Кагуя не сильно злится, а чел не сильно злит
    if -0.1 < USERS[usr_id]['mood'] < 0 and emo_rate >= 0:
        # прощаем ему всю хуйню
        USERS[usr_id]['mood'] = 0
    log(bot.message)
    if random.random() <= 0.01:  # имба, редкость
        ...
        if clear_msg(bot.message.text)[:5] == 'Скажи':
            ...
            if USERS[usr_id]['mood'] < 0:
                rep = NEGATIVE_WHY_ANSWERS[random.randint(
                    0, len(NEGATIVE_WHY_ANSWERS) - 1)]
            else:
                rep = POSITIVE_WHY_ANSWERS[random.randint(
                    0, len(POSITIVE_WHY_ANSWERS) - 1)]
        elif bot.message.text.lower() in APPEALS:
            if USERS[usr_id]['mood'] < 0:
                rep = NEGATIVE_APPEALS_ANSWERS[random.randint(
                    0, len(NEGATIVE_APPEALS_ANSWERS) - 1)]
            else:
                rep = POSITIVE_APPEALS_ANSWERS[random.randint(
                    0, len(POSITIVE_APPEALS_ANSWERS) - 1)]
            ...
            if hour < NIGHT_START and hour >= MORNING_START:
                rep = HAVE_A_GOOD_DAY[random.randint(
                    0, len(HAVE_A_GOOD_DAY) - 1)]
            else:
                rep = HAVE_A_GOOD_NIGHT[random.randint(
                    0, len(HAVE_A_GOOD_NIGHT) - 1)]
        elif bot.message.text.lower() in WHATSUP_QUESTIONS or bot.message.text.lower() + '?' in WHATSUP_QUESTIONS:
            if USERS[usr_id]['mood'] < 0:
                rep = NEGATIVE_WAHATSUP_ANSWERS[random.randint(
                    0, len(NEGATIVE_WAHATSUP_ANSWERS) - 1)]
            else:
                rep = POSITIVE_WAHATSUP_ANSWERS[random.randint(
                    0, len(POSITIVE_WAHATSUP_ANSWERS) - 1)]
        elif USERS[usr_id]['mood'] < 0:
            if '?' in bot.message.text:
                rep = NEGATIVE_QUIESTION_ANSWERS[random.randint(
                    0, len(NEGATIVE_QUIESTION_ANSWERS) - 1)]
            else:
                if random.random() <= REPLY_WITH_USR_MSG and len(USERS[usr_id]['top_messages']) > 0 and len(USERS[usr_id]['max_rating_neg_msgs']) > 0:
                    attempts = 0
                    rep = USERS[usr_id]['max_rating_neg_msgs'][random.randint(
                        0, len(USERS[usr_id]['max_rating_neg_msgs']) - 1)]
                    while attempts <= 5 and time.time() - USERS[usr_id]['top_messages'][rep]['time'] <= 30:
                        rep = USERS[usr_id]['max_rating_neg_msgs'][random.randint(
                            0, len(USERS[usr_id]['max_rating_neg_msgs']) - 1)]
                        attempts += 1
                    if rep == '':
                        rep = NEGATIVE_REPLIES[random.randint(
                            0, len(NEGATIVE_REPLIES) - 1)]
                else:
                    rep = NEGATIVE_REPLIES[random.randint(
                        0, len(NEGATIVE_REPLIES) - 1)]
        else:
            if '?' in bot.message.text:
                rep = POSITIVE_QUESTION_ANSWERS[random.randint(
                    0, len(POSITIVE_QUESTION_ANSWERS) - 1)]
            else:
                if random.random() <= REPLY_WITH_USR_MSG and len(USERS[usr_id]['top_messages']) > 0 and len(USERS[usr_id]['max_rating_pos_msgs']) > 0:
                    attempts = 0
                    rep = USERS[usr_id]['max_rating_pos_msgs'][random.randint(
                        0, len(USERS[usr_id]['max_rating_pos_msgs']) - 1)]
                    while attempts <= 5 and time.time() - USERS[usr_id]['top_messages'][rep]['time'] <= 30:
                        rep = USERS[usr_id]['max_rating_pos_msgs'][random.randint(
                            0, len(USERS[usr_id]['max_rating_pos_msgs']) - 1)]
                        attempts += 1
                    if rep == '':
                        rep = POSITIVE_REPLIES[random.randint(
                            0, len(POSITIVE_REPLIES) - 1)]
                else:
                    rep = POSITIVE_REPLIES[random.randint(
                        0, len(POSITIVE_REPLIES) - 1)]
        time.sleep(SLEEP_TIME)
...
def whoami(bot, update):  # отвечает на "Кто я сегодня?"
    ...
    if time.time() - USERS[usr_id]['last_usage'] > CRITICAL_LAST_USAGE_TIME:
        greeting_to_unseen_user(bot.message)
    if USERS[usr_id]['mood'] < 0:
        rep = NEGATIVE_WHOAMI_REPLIES[random.randint(
            0, len(NEGATIVE_WHOAMI_REPLIES) - 1)]
    else:
        rep = POSITIVE_WHOAMI_REPLIES[random.randint(
            0, len(POSITIVE_WHOAMI_REPLIES) - 1)]
    time.sleep(SLEEP_TIME)
    bot.message.reply_text('{}, ты сегодня такой {}'.format(
        bot.message.chat.first_name, rep))
    USERS[usr_id]['last_usage'] = time.time()
    write_users()
...
def dorandom(bot, update):  # отвечает на "Рандомчик"
    ...
    if USERS[usr_id]['rand_max'] == 0:
        bot.message.reply_text(
            'Хорошо, я должна назвать рандомное число от 1 до .. ?')
        USERS[usr_id]['waiting_for_random'] = True
    else:
        try:
            rand = random.randint(1, USERS[usr_id]['rand_max'])
            CONTROL_MSGS[get_id(bot)].reply_text(
                'Не знаю, зачем тебе это, но пусть будет ' + str(rand) + '.')
        except Exception:
...
def sendday(bot, update):  # отвечает на "Какой сегодня день?"
    ...
    if USERS[usr_id]['mood'] < 0:
        ...
        rep = 'Хорошего дня'
    bot.message.reply_text('Это, кстати, {} {}. {}, {})'.format(
        date.today().day, MONTHS[date.today().month-1], rep, bot.message.chat.first_name))
    USERS[usr_id]['last_usage'] = time.time()
    write_users()
...
def whensmoketime(bot, update):  # когда там перекур
    ...
    if USERS[usr_id]['group'] == '' or today_pairs_nums == None:
        if USERS[usr_id]['show_set_group_notice']:
            time.sleep(SLEEP_TIME)
            bot.message.reply_text(
                '(Кстати, ты можешь указать свою группу при помощи команды /set_group [группа] и я буду показывать тебе более точную информацию о перекурах)')
    ...
    for p_num in today_pairs_nums:
        ...
        if p_end >= curr_time >= p_start:
            curr_state = 'на паре'
            pair_num = p_num
            time_to_smoke = datetime.datetime.combine(datetime.date.today(
            ), p_end) - datetime.datetime.combine(datetime.date.today(), curr_time)
            time_to_smoke_seconds = time_to_smoke.seconds + \
                round(time_to_smoke.microseconds/10**6)
            time_to_smoke_minutes = round(
                (time_to_smoke.seconds + round(time_to_smoke.microseconds/10**6)) / 60)
            ...
            if next_p_start >= curr_time >= p_end:
                curr_state = 'перекур'
                time_to_next_pair = datetime.datetime.combine(datetime.date.today(
                ), next_p_start) - datetime.datetime.combine(datetime.date.today(), curr_time)
                time_to_next_pair_seconds = time_to_next_pair.seconds + \
                    round(time_to_next_pair.microseconds/10**6)
                time_to_next_pair_minutes = round(
                    (time_to_next_pair.seconds + round(time_to_next_pair.microseconds/10**6)) / 60)
    ...
    if curr_state == 'на паре':
        ...
        if time_to_smoke_minutes >= 60:
            ...
            if time_to_smoke_minutes == 0:
                ...
                bot.message.reply_text('До перекура 1 час {} {}'.format(
                    time_to_smoke_minutes, get_minutes_declension(time_to_smoke_minutes)))
        elif time_to_smoke_seconds < 60:
            bot.message.reply_text('До перекура {} {}'.format(
                time_to_smoke_seconds, get_seconds_declension(time_to_smoke_seconds)))
        else:
            bot.message.reply_text('До перекура {} {}'.format(
                time_to_smoke_minutes, get_minutes_declension(time_to_smoke_minutes)))
        ...
        time.sleep(SLEEP_TIME)
        bot.message.reply_text(
            'Учись усерднее, {}'.format(bot.message.chat.first_name))

        update.bot.send_photo(chat_id=bot.message.chat.id,
                              photo=open('SMOKETIME/learntime.jpg', 'rb'))
        ...
        if time_to_next_pair != -1:
            if time_to_next_pair_seconds < 60:
                bot.message.reply_text('До начала пары {} {}'.format(
                    time_to_next_pair_seconds, get_seconds_declension(time_to_next_pair_seconds)))
            else:
                bot.message.reply_text('До начала пары {} {}'.format(
                    time_to_next_pair_minutes, get_minutes_declension(time_to_next_pair_minutes)))

        update.bot.send_photo(chat_id=bot.message.chat.id,
                              photo=open('SMOKETIME/smoketime.jpg', 'rb'))
    else:
        time.sleep(SLEEP_TIME)
        bot.message.reply_text('Чел, ты не на парах')

        update.bot.send_photo(chat_id=bot.message.chat.id,
                              photo=open('SMOKETIME/notyet.png', 'rb'))

        nearest_pair = get_nearest_pair_time(
            bot.message, datetime.date.today())
        if nearest_pair != None:
            time.sleep(SLEEP_TIME)
            bot.message.reply_text('Ближайшая пара {}'.format(
                nearest_pair.strftime("%d.%m в %H:%M")))
...
def whentogetup(bot, update):
    ...
    if USERS[usr_id]['base_get_up_time_hour'] == None or USERS[usr_id]['base_get_up_time_minute'] == None:
        ...
        base_get_up_time = datetime.time(
            USERS[usr_id]['base_get_up_time_hour'], USERS[usr_id]['base_get_up_time_minute'])
        if datetime.datetime.now().hour < NIGHT_LIMIT:
            tomorrow_pairs_nums = get_pairs_nums(
                bot.message, datetime.date.today())
        else:
            tomorrow_pairs_nums = get_pairs_nums(
                bot.message, datetime.date.today() + datetime.timedelta(days=1))
        if tomorrow_pairs_nums == None:
            time.sleep(SLEEP_TIME)
            bot.message.reply_text('Я не знаю, какие пары у тебя завтра. Похуй, вставай в {}'.format(
                base_get_up_time.strftime("%H:%M")))
            if USERS[usr_id]['show_set_group_notice']:
                time.sleep(SLEEP_TIME)
                bot.message.reply_text(
                    '(Если хочешь, чтобы я знала, когда у тебя пары, укажи свою группу через команду /set_group [группа])')
            ...
            get_up_time = datetime.datetime.combine(datetime.date.today(
            ), PAIRS_TIME[first_pair]['start']) - datetime.datetime.combine(datetime.date.today(), PAIRS_TIME[1]['start'])
            get_up_time += datetime.datetime.combine(
                datetime.date.today(), base_get_up_time)
...
# на случай если челик переобулся и захотел сменить город
...
def sendweather(bot, update):  # отправляет погоду
    ...
    try:
        ...
        rep = 'В городе ' + USERS[usr_id]['city'] + \
            ' сейчас ' + str(round(w[0]["temp"])) + '°C, ' + w[1]
...
# вызывается при вопросе про погоду, имба функция
...
def main():  # БАЗА
    ...
    print('Started')
    bot = Updater(
        "5260290537:AAGWg9J4a5dZDqsrq3MG3fejuBvD-0tasOA", use_context=True)
    ...
    bot.dispatcher.add_handler(MessageHandler(
        Filters.regex('Кто я сегодня?'), whoami))
    bot.dispatcher.add_handler(MessageHandler(
        Filters.regex('Скинь ножки'), sendlegs))
    bot.dispatcher.add_handler(MessageHandler(
        Filters.regex('Рандомчик'), dorandom))
    bot.dispatcher.add_handler(MessageHandler(
        Filters.regex('Какой сегодня день?'), sendday))
    bot.dispatcher.add_handler(MessageHandler(
        Filters.regex('Сколько до перекура?'), whensmoketime))
    bot.dispatcher.add_handler(MessageHandler(Filters.regex(
        'Во сколько мне завтра вставать?'), whentogetup))
    bot.dispatcher.add_handler(MessageHandler(
